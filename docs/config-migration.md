# 配置迁移：从旧 DB 导出 projects → 写入 `projects.yaml`

本项目已将 **projects / repos 设置**从 DB-backed 改为 **YAML file-first**。服务端/前端不会写入 `config.yaml` / `projects.yaml` / `secret.env`；如需从旧 DB 迁移数据，请使用一次性脚本完成导出/合并。

> 说明：你如果在 `~/.config/vk/projects.yaml` 里只看到了 `projects:`，这是正常的——该导出命令只处理 DB 里的 projects/repos。
...
- `config_dir` / `config_yaml` / `projects_yaml` / `secret_env`：用户配置目录与目标文件（优先 `VK_CONFIG_DIR`；否则 OS 默认目录）

## secrets 迁移建议（重要）

- 脚本 **不会**导出任何 token/pat/key。
- 建议将 secrets 放入与 `config.yaml` 同级的 `secret.env`，再在 `config.yaml` / `projects.yaml` 中通过 `{{secret.NAME}}` 引用。
...
已知约束：
- 服务端/前端不允许写入 config.yaml/projects.yaml/secret.env；只能用一次性脚本/CLI 迁移。
