...
**Goals:**
- 引入独立 operator CLI：`vk`，承载迁移与其它“非 server runtime”的一次性命令。
...
1. **迁移工具剥离为独立 operator CLI：`vk`**
   - 选择：新增 `crates/vk`（命令行入口）并提供 `vk` 二进制，迁移命令以子命令形式承载：\n     - `vk migrate export-db-projects-yaml`\n     - `vk migrate export-asset-config-yaml`\n     - `vk migrate prompt`\n     （以及后续可扩展的其它 operator tooling，例如 schema upsert）\n   - 选择：主 `server` 移除 legacy 子命令 wiring，避免编译/依赖拖累。\n   - 说明：`vk` 作为统一入口的 CLI，不在本变更中强制要求把 `server` 也包成子命令，但预留未来扩展空间（例如 `vk server`）。
...
1. 新增 `vk` crate/bin，先把现有命令原样搬过去（保持行为）。
