...
- [x] 8.2 清理剩余 flaky 测试：移除脆弱 sleep/墙钟阈值断言；确保 env 修改使用 RAII guard，避免 panic 后污染后续用例
- [x] 8.3 文案/指引检查：确保用户提示统一指向 `projects.yaml` / `projects.d/*.yaml` 的最新配置路径（避免过时指向 `config.yaml` only）
