...
1. **Projects DB 同步策略：从“reload 时同步”改为“按需最小化写入或完全移除”**
   - 选择：移除 `AppRuntime::sync_config_projects_to_db()` 的 reload 调用；若 DB 仍需要 project 记录（外键/历史），改为在“创建 task/attempt 时”按需 `find_or_create(project_id, name)` 写入最小行。
   - 原因：避免“文件变更触发 DB 写入”的隐式副作用与双源漂移；让写入路径只在运行态真实需要时发生。
...
5. **schema 生成改为 CLI upsert**
   - 选择：新增 CLI 命令（例如 `server config schema upsert`）在 config dir 写入/更新 `config.schema.json` 与 `projects.schema.json`；服务启动路径移除写盘行为。
   - 原因：减少运行态副作用与权限问题；更符合“最小 core server”。
...
2. 调整 projects DB 同步：移除 reload sync，改为按需最小写入（或完全不写入，视 DB 约束而定）。
3. 引入 `ProjectPublic` DTO 并切换 `/api/projects` 与相关 WS/补丁流（如适用）。
4. 增加 CLI schema upsert，移除启动写盘；更新 UI 指引。
5. 补齐回归测试与文档，发布。

## Open Questions

- DB project 表是否存在强外键约束（task -> project）？若存在，按需写入最小行将成为必须步骤。
- schema upsert 命令的归属：留在 `server` 二进制还是后续迁移到 `vk-migrate`？
