...
- [ ] 1.1 Add DB migration to extend `task_groups` with milestone columns: `objective`, `definition_of_done`, `default_executor_profile_id`, `automation_mode` (default `manual`), plus a scheduler enqueue field for “run next step” (for example `run_next_step_requested_at`). Verification: `pnpm run prepare-db && cargo test -p db`
...
- [ ] 2.3 Add a focused API action to enqueue “run next step” for a milestone (run once), consumed by the scheduler so concurrency rules are preserved. Return a structured response describing what was enqueued (or why nothing was eligible). Verification: `cargo test -p server`
...
- [ ] 3.2 Update auto-orchestration candidacy/diagnostics so grouped node tasks are dispatchable when milestone automation is enabled (even if the project is manual), unless a task-level manual override is set. Non-milestone grouped tasks remain unscheduled with an explicit reason. Verification: `cargo test --workspace`
...
- [ ] 4.3 Add a “Run next step” control wired to the enqueue API action (not direct start), with clear disabled states and diagnostics (blocked by deps, checkpoint gate, already running). Verification: `pnpm run check && browser smoke`
