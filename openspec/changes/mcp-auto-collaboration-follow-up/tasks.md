...
- [ ] 1.1 Add persisted control-transfer reasons to task orchestration state and expose them in shared task DTOs. Verification: `cargo test -p server && pnpm run generate-types`
- [ ] 1.2 Add project-scoped executor/profile policy for auto-managed work, including config migration/defaulting and structured rejection diagnostics. Verification: `cargo test -p server && cargo test --workspace`

## 2. MCP review handoff contracts

- [ ] 2.1 Add a focused MCP read tool for review-ready handoff payloads with explicit output schema coverage. Verification: `cargo test -p server mcp::task_server::tools`
