...
The system SHALL provide a standalone migration CLI binary (named `vk-migrate`) that can be built and run without npm tooling and without starting the server.

#### Scenario: Build vk-migrate alongside the server
- **WHEN** the workspace is built from source using Cargo
- **THEN** the `vk-migrate` binary can be produced and runs with `--help`
