...
Task orchestration reads SHALL expose continuation counters and continuation stop reasons when bounded continuation is enabled.
...
- **THEN** the response includes the continuation count, remaining budget if available, and the latest continuation stop reason
