...
- [ ] 1.1 修改 `scripts/prepare-db.js`：临时 sqlite 放 OS temp + 随机路径（或唯一目录），避免在 repo 内创建固定文件
- [ ] 1.2 增加清理逻辑（成功/失败/中断后尽量清理临时文件）
...
- [ ] 2.1 移除 `scripts/qa.sh` 中重复的 migration check（合并 `prepare-db:check` 与 `remote:prepare-db:check` 逻辑）
- [ ] 2.2 确保 CI 仍覆盖关键检查（types、lint、tests）
...
- [ ] 4.1 将 `scripts/check-i18n.sh` 重写为单个 Node/TS 脚本，移除 jq/perl/diff 依赖
- [ ] 4.2 更新 `package.json` scripts 与 CI 调用点
...
- [ ] 5.1 新增 `crates/test-support`：提供 env lock、EnvVarGuard（RAII）、TempRoot、TestDb 等通用能力
- [ ] 5.2 迁移现有重复实现：优先 `crates/server`/`crates/config`/`crates/app-runtime`（减少 flaky 与重复）
- [ ] 5.3 修复遗留 guard：让需要 env lock 的 guard 自己拿锁（避免调用方遗漏）
