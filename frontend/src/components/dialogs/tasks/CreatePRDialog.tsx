...
const CreatePRDialogImpl = NiceModal.create<CreatePRDialogProps>(
  ({ attempt, task, repoId, targetBranch }) => {
    ...
    const selectedAgent = config?.executor_profile?.executor ?? null;
    const { data: cliPreflight, isLoading: preflightLoading } =
      useCliDependencyPreflight(selectedAgent, modal.visible && !loading);
                ...
                {selectedAgent && (
                  <Alert
                    variant={
                      ...
                      cliPreflight.github_cli.type !== 'ready'
                        ? 'destructive'
                        : 'default'
                    }
                  >
                    ...
                    <AlertDescription className="space-y-1">
                      <p>
                        ...
                        {preflightLoading
                          ...
                          : cliPreflight?.github_cli.type === 'ready'
                            ? t('createPrDialog.preflight.githubReady')
                            : cliPreflight?.github_cli.type === 'not_installed'
                              ...
                              : cliPreflight?.github_cli.type ===
                                ...
                                : cliPreflight?.github_cli.type === 'error'
                                  ? t('createPrDialog.preflight.githubError')
                                  : null}
                      </p>
                      {autoGenerateDescription && (
                        <p>
                          {t('createPrDialog.preflight.agentLabel')}{' '}
                          {preflightLoading
                            ? t('createPrDialog.preflight.checking')
