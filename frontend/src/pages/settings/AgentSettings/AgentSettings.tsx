...
import { useProfiles } from '@/hooks/config/useProfiles';
...
export function AgentSettings() {
  ...
  const selectedAgentConfig = (selectedVariantConfig?.[selectedExecutorType] ??
  ...
  const commandVersion = baseCommandOverride
    ? null
    : (commandInfo?.version ?? null);
  ...
  const handleSyncLlman = async (path: string) => {
    ...
    try {
      if (config && (config.llman_claude_code_path ?? null) !== nextPath) {
      ...
      await profilesApi.importLlman();
      reloadSystem();
      await refetch();
    } catch (err) {
      setSaveError(t('settings.agents.llman.importError'));
