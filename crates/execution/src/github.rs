...
use repos::{GitHubRepoInfo, GitHubRepoInfoError};
use serde::Serialize;
use thiserror::Error;
use tokio::task;
use tracing::info;
...
pub use cli::{GhCliError, PrCommentAuthor, ReviewCommentUser};
...
impl From<GitHubRepoInfoError> for GitHubServiceError {
    fn from(error: GitHubRepoInfoError) -> Self {
        GitHubServiceError::Repository(error.to_string())
    }
}
