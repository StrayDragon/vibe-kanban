...
pub use local::LocalContainerService;
...
fn parse_log_persistence_mode_env() -> Option<LogPersistenceMode> {
    ...
    match value.as_str() {
        ...
        "legacy_jsonl" | "legacy-jsonl" | "jsonl" | "legacy" => {
            Some(LogPersistenceMode::LegacyJsonl)
        }
        _ => {
            tracing::warn!(
...
pub trait ContainerService {
    ...
    async fn cleanup_legacy_jsonl_logs(&self) -> Result<(), ContainerError> {
        ...
        let deleted =
            ExecutionProcessLogs::delete_legacy_for_completed_before(&self.db().pool, cutoff)
                .await?;
        if deleted > 0 {
            tracing::info!(
            ...
            && let Some(store) = self.get_msg_store_by_id(&execution_process.id).await
        {
            ...
            if !history_meta.evicted {
                let (entries, has_more) = match channel {
                    ...
                    LogEntryChannel::Normalized => {
                        store.normalized_history_page(limit, cursor_usize)
                    }
                };
            ...
            if cursor_before_min && let Ok((entries, has_more)) = fetch_db_entries(cursor).await {
                return Ok(LogHistoryPageData {
                    entries,
                    has_more,
                    history_truncated,
    ...
    async fn start_workspace(
        ...
        if let Some(milestone_id) = task.milestone_id {
            match Milestone::find_by_id(&self.db().pool, milestone_id).await {
                Ok(Some(milestone)) => {
                    let prompt_with_milestone = append_milestone_context_to_prompt(
                        &prompt,
                        milestone.objective.as_deref(),
                        milestone.definition_of_done.as_deref(),
                    );
                    ...
                    if let Some(node_id) = task.milestone_node_id.as_deref() {
                            ...
                            match node.and_then(|node| node.instructions.as_deref()) {
                                Some(instructions) if !instructions.trim().is_empty() => {
                                    tracing::info!(
                                        ...
                                        milestone_id = %milestone_id,
                                        node_id = %node_key,
                                        "Appending milestone node instructions to prompt"
                                    );
                                    prompt =
                                ...
                                _ => {
                                    tracing::info!(
                                        ...
                                        milestone_id = %milestone_id,
                                        node_id = %node_key,
                                        "No milestone node instructions to append"
                                    );
                                }
                            }
                        }
                ...
                Ok(None) => {
                    tracing::warn!(
                        task_id = %task.id,
                        milestone_id = %milestone_id,
                        "Milestone not found while building task prompt"
                    );
                }
                Err(err) => {
                    tracing::warn!(
                        ...
                        milestone_id = %milestone_id,
                        error = %err,
                        "Failed to load milestone while building task prompt"
                    );
                }
            }
        }
