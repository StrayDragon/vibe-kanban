...
use executor_codex::codex::normalize_logs::{Approval, normalize_logs};
use executors_core::{
...
enum FakeCommand {
    ...
    Approve {
        call_id: String,
        tool_name: String,
        status: utils_core::approvals::ApprovalStatus,
    },
}
...
fn parse_approve_command(rest: &str) -> Result<FakeCommand, FakeAgentError> {
    ...
    let status = match status_token.as_str() {
        "approved" | "approve" | "ok" => utils_core::approvals::ApprovalStatus::Approved,
        "pending" => utils_core::approvals::ApprovalStatus::Pending,
        "denied" | "deny" => {
            let reason = parts.collect::<Vec<_>>().join(" ");
            utils_core::approvals::ApprovalStatus::Denied {
        ...
        "timed_out" | "timedout" | "timeout" => utils_core::approvals::ApprovalStatus::TimedOut,
        _ => {
