...
use ts_rs::TS;
use utils_core::{approvals::ApprovalStatus, diff::create_unified_diff, path::make_path_relative};
...
impl ClaudeLogProcessor {
    ...
    fn normalize_claude_tool_result_value(
    ...
    ) -> (executors_core::logs::ToolResultValueType, serde_json::Value) {
        if let Some(s) = content.as_str() {
            if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(s) {
                return (executors_core::logs::ToolResultValueType::Json, parsed);
            }
            return (
                executors_core::logs::ToolResultValueType::Markdown,
        ...
        (
            executors_core::logs::ToolResultValueType::Json,
            content.clone(),
        )
    }
                ...
                for item in &message.content {
                        ...
                        && let Some(info) = self.tool_map.get(tool_use_id).cloned()
                    {
                        ...
                        if is_command {
                            ...
                            {
                                Some(executors_core::logs::CommandRunResult {
                                    exit_status: Some(
                                        executors_core::logs::CommandExitStatus::ExitCode {
                                ...
                                Some(executors_core::logs::CommandRunResult {
                                    exit_status: (*is_error).map(|is_error| {
                                        executors_core::logs::CommandExitStatus::Success {
                                            success: !is_error,
                                        }
                                    }),
                            ...
                            let entry = NormalizedEntry {
                                ...
                                entry_type: NormalizedEntryType::ToolUse {
                                    ...
                                    action_type: ActionType::Tool {
                                        ...
                                        result: Some(executors_core::logs::ToolResult {
                                            r#type: res_type,
                                            value: res_value,
                                        }),
                                    },
...
#[cfg(test)]
mod tests {
    use executors_core::logs::utils::{
        EntryIndexProvider, patch::extract_normalized_entry_from_patch,
    };
    ...
    async fn test_streaming_patch_generation() {
        ...
        let patch_count = history
            .iter()
            .filter(|msg| matches!(msg, logs_protocol::LogMsg::JsonPatch(_)))
            .count();
        assert!(
            patch_count > 0,
