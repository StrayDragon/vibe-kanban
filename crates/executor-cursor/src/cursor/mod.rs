...
impl StandardCodingAgentExecutor for CursorAgent {
    ...
    fn normalize_logs(&self, msg_store: Arc<MsgStore>, worktree_path: &Path) {
        ...
        tokio::spawn(async move {
            let mut stderr = msg_store_stderr.clone().stderr_chunked_stream();
        ...
        tokio::spawn(async move {
            let mut lines = msg_store.clone().stdout_lines_stream();
                            ...
                            && let Some(&idx) = call_index_map.get(cid)
                        {
                            ...
                            if let CursorToolCall::Shell { args, result } = &tool_call {
                                ...
                                let exit_status = exit_code.map(|code| {
                                    executors_core::logs::CommandExitStatus::ExitCode { code }
                                });
                                new_action = ActionType::CommandRun {
                                    command: args.command.clone(),
                                    result: Some(executors_core::logs::CommandRunResult {
                                        exit_status,
                                        output,
                                    }),
                                };
                                ...
                                new_action = ActionType::Tool {
                                    ...
                                    result: md.map(|s| executors_core::logs::ToolResult {
                                        r#type: executors_core::logs::ToolResultValueType::Markdown,
...
mod tests {
    ...
    async fn test_cursor_streaming_patch_generation() {
        ...
        let patch_count = history
            .iter()
            .filter(|m| matches!(m, logs_protocol::LogMsg::JsonPatch(_)))
            .count();
        assert!(
            patch_count >= 2,
