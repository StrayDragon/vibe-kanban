...
pub struct Config {
    ...
    #[serde(default = "default_git_no_verify")]
...
impl Config {
    fn from_v8_config(old_config: v8::Config) -> Self {
        Self {
            ...
            git_no_verify: old_config.git_no_verify,
...
mod tests {
    ...
    #[test]
    fn migrates_from_v8_with_llman_path_none() {
        let v8_config = v8::Config::default();
        let raw = serde_json::to_string(&v8_config).expect("serialize v8 config");
        let v9_config = Config::from(raw);

        assert_eq!(v9_config.config_version, "v9");
        assert!(v9_config.llman_claude_code_path.is_none());
    }
}
