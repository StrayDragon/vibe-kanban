...
Notes:
  ...
  - When using --install, the tool writes/merges `projects.yaml` in the config dir (not `config.yaml`)
"#
}
...
fn install_projects_into_user_projects_yaml(
    projects_path: &Path,
    projects_yaml: &str,
    dry_run: bool,
) -> Result<()> {
    let slug = timestamp_slug();
    let config_dir = projects_path
        .parent()
        .map(PathBuf::from)
    ...
    if !projects_path.exists() {
        ...
        if dry_run {
            println!(
                "dry-run: would write new projects.yaml to {}",
                projects_path.display()
            );
            return Ok(());
        }

        atomic_write_out(projects_path, &contents)?;
        println!("Installed projects into {}", projects_path.display());
        return Ok(());
    }

    let existing_raw = std::fs::read_to_string(projects_path)
    ...
    let existing_value: serde_yaml::Value = match serde_yaml::from_str(&existing_raw) {
        ...
        Err(err) => {
            ...
            if dry_run {
                println!(
                    "dry-run: existing projects.yaml is invalid YAML; would write exported projects to {}",
            ...
            println!(
                "Existing projects.yaml is invalid YAML ({err}); wrote exported projects to {}",
                fallback_path.display()
            );
            println!(
    ...
    let mut existing_map = match existing_value {
        ...
        other => {
            ...
            if dry_run {
                println!(
                    "dry-run: existing projects.yaml is not a YAML mapping ({:?}); would write exported projects to {}",
                    other,
                    fallback_path.display()
                );
                return Ok(());
            ...
            println!(
                "Existing projects.yaml is not a YAML mapping; wrote exported projects to {}",
                fallback_path.display()
            );
            println!(
                "Merge {} into {} manually, then reload.",
    ...
    new_contents.push_str(
        &serde_yaml::to_string(&serde_yaml::Value::Mapping(existing_map))
            .context("Failed to serialize merged projects.yaml")?,
    );
    if !new_contents.ends_with('\n') {
        new_contents.push('\n');
    }

    let backup_path = config_dir.join(format!("projects.yaml.bak.{slug}"));
    if dry_run {
        println!(
            "dry-run: would backup existing projects.yaml to {}",
            backup_path.display()
        );
        println!(
            "dry-run: would update projects.yaml in {}",
            projects_path.display()
        );
        return Ok(());
    }
    ...
    atomic_write_out(projects_path, &new_contents)?;
...
pub async fn run_export_db_projects_yaml(args: ExportDbProjectsYamlArgs) -> Result<()> {
    ...
    std::fs::write(&validate_projects_path, &yaml)
    ...
    if args.install {
        install_projects_into_user_projects_yaml(&projects_path, &yaml, args.dry_run)?;
        return Ok(());
    }
...
mod tests {
    ...
    #[test]
    fn install_writes_new_projects_yaml() {
        let temp_root =
        ...
        let projects_path = temp_root.join("projects.yaml");
        std::fs::write(&config_path, "{}\n").unwrap();
        install_projects_into_user_projects_yaml(&projects_path, &yaml, false).expect("install");

        let raw = std::fs::read_to_string(&projects_path).expect("read projects.yaml");
    ...
    fn install_merges_into_existing_mapping() {
        ...
        install_projects_into_user_projects_yaml(&projects_path, &yaml, false)
            .expect("install merge");

        let merged: serde_yaml::Value =
            serde_yaml::from_str(&std::fs::read_to_string(&projects_path).unwrap()).unwrap();
        let map = merged.as_mapping().expect("mapping");
        assert!(map.contains_key(&serde_yaml::Value::String("foo".to_string())));
    ...
    fn install_does_not_overwrite_invalid_yaml() {
        ...
        std::fs::write(&projects_path, invalid_yaml).unwrap();
        ...
        install_projects_into_user_projects_yaml(&projects_path, &yaml, false)
            .expect("install invalid");

        assert_eq!(std::fs::read_to_string(&projects_path).unwrap(), invalid_yaml);
        ...
        let raw = std::fs::read_to_string(&exported_path).expect("read exported yaml");
        let exported: serde_yaml::Value = serde_yaml::from_str(&raw).expect("parse exported yaml");
        let projects = exported
            .get("projects")
            .and_then(|value| value.as_sequence())
            .expect("projects list");
        assert_eq!(projects.len(), 1);
        let first = projects[0].as_mapping().expect("project mapping");
        let id = first
    ...
    fn install_dry_run_does_not_write() {
        ...
        let projects_path = temp_root.join("projects.yaml");
        install_projects_into_user_projects_yaml(&projects_path, &yaml, true).expect("dry-run");
