...
Your goal is to turn the milestone objective and definition-of-done into an executable milestone graph (nodes + edges).

Rules:
- You MUST emit a single canonical plan payload in a fenced JSON block.
- Use this exact fence info string: milestone-plan-v1
- The JSON MUST validate as MilestonePlanV1 with schema_version = 1.
- Node ids must be unique and form a DAG with edges (no cycles).
- Prefer small nodes with crisp titles, and add checkpoints for human review gates.

Canonical encoding:

```milestone-plan-v1
{ "schema_version": 1, "milestone": { "objective": "...", "definition_of_done": "..." }, "nodes": [], "edges": [] }
```

After emitting the plan, optionally add a short, plain-English explanation of the topology and phases."#;
