...
- `crates/`: Rust workspace crates — `server` (API + bins), `db` (SeaORM models/migrations), `executors`, `services`, `utils`, `deployment`, `local-deployment`, `remote`.
...
- Prepare database schema (SeaORM migrations): `pnpm run prepare-db`
